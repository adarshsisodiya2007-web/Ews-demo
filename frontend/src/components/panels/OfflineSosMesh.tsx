        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {sosLedger.map(sos => (
              ...
              >
                <div>
                  <strong style={{ color: '#ef4444' }}>{sos.id}</strong> · {sos.people} Person{sos.people > 1 ? 's' : ''} {sos.urgent ? '(MEDICAL URGENT)' : ''} · Lat: {sos.lat.toFixed(4)}, Lon: {sos.lon.toFixed(4)}
                  <span style={{ color: '#64748b', marginLeft: '8px' }}>({sos.time})</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{
                    background: sos.status === 'SYNCHRONIZED_TO_COMMAND_CENTER' ? 'rgba(34,197,94,0.2)' : 'rgba(245,158,11,0.2)',
                    color: sos.status === 'SYNCHRONIZED_TO_COMMAND_CENTER' ? '#4ade80' : '#fcd34d',
