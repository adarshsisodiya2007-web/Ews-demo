...
public class CitizenAuthController {
    ...
    public ResponseEntity<?> sendOtp(@Valid @RequestBody SendOtpRequest req) {
        ...
        Optional<AppUser> userOpt = userRepo.findByPhone(normalizedPhone)
                .or(() -> userRepo.findByUsername(normalizedPhone));

        // If in production (non-demo) or demo mode without matching registered user
        if (userOpt.isEmpty()) {
            log.warn("Citizen send-otp rejected: Phone {} is not registered", normalizedPhone);
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of(
                    "success", false,
                    "registered", false,
                    "message", "This mobile number is not registered. Please check the number and try again."
            ));
        }
    ...
    public ResponseEntity<?> registerCitizen(@Valid @RequestBody SendOtpRequest req) {
        ...
        log.info("New citizen registered with phone: {}", normalizedPhone);

        // Send OTP right away after registration
        otpService.generateAndSendOtp(normalizedPhone, "Your SATARK Citizen verification code is: %s. Valid for 5 minutes.");
    ...
    public ResponseEntity<?> verifyOtp(@Valid @RequestBody VerifyOtpRequest req) {
        ...
        AppUser user = userRepo.findByPhone(normalizedPhone)
                .or(() -> userRepo.findByUsername(normalizedPhone))
                .orElseThrow(() -> new IllegalStateException("User account not found. Please register first."));
